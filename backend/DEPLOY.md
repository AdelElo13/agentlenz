# Deploying AgentLenz API to Fly.io
...
> Change `agentlenz-api` to a globally unique name if that one is taken.
...
```bash
fly postgres attach agentlenz-db --app agentlenz-api
```
...
fly secrets set SECRET_KEY=your-value-here --app agentlenz-api
```
...
#   fly proxy 5432 -a agentlenz-db
# Then in this terminal:
export DATABASE_URL="postgresql://postgres:<password>@localhost:5432/agentlenz_api"
...
```bash
fly ssh console --app agentlenz-api
...
| Task | Command |
|---|---|
| View logs | `fly logs --app agentlenz-api` |
| Open a shell | `fly ssh console --app agentlenz-api` |
| Connect to Postgres | `fly postgres connect -a agentlenz-db` |
| Scale down to zero | `fly scale count 0 --app agentlenz-api` |
| Check app status | `fly status --app agentlenz-api` |
